...
class LidarCloudDataset(Dataset):
    def __init__(self, file_list: List[Tuple[str, str]], preload: bool = True, augment: bool = False, norm_stats: dict | None = None):
        self.samples: List[Tuple[str, str]] = file_list
    ...
    def _augment(self, x, y):
        ...
        if len(ch_indices) > 0 and np.random.rand() < CH_GAMMA_P:
            ...
            # 避免负值；对[0,1]做幂运算
            for ch in enumerate(ch_indices):
...
def predict_full_image_with_stitching(model, x_full, window_w, device):
    ...
    # 概率图用于累加每个窗口的预测概率
    # 权重图用于记录每个像素被预测了多少次（或以多大权重）
    full_pred_prob = torch.zeros((H, W_full), device=device)
    sum_weights = torch.zeros((H, W_full), device=device)

    # 2. 定义滑动步长和重叠区域
    # 步长设为窗口宽度的 1/2，这样每个窗口都有50%的重叠区域
    stride = window_w // 2
    
    # 3. 定义一个三角权重窗口，用于平滑拼接
    # 窗口中心权重为1，边缘权重为0，线性过渡。
    # 这使得拼接时，重叠区域的贡献是平滑的，避免出现明显的拼接缝。
    ...
    for start in range(0, W_full, stride):
        ...
        if pad_w > 0:
            # 注意：这里需要将numpy array转为tensor才能使用F.pad
...
def visualize_and_save(model, base_dataset, epoch=0, prefix=''):
    ...
    for idx in indices:
        ...
        plt.figure(figsize=(18, 6)) # 增大图像尺寸以容纳完整图像
...
def main():
    ...
    VAL_STATION = '58847'  # 指定一个站点用于验证
    TRAIN_STATIONS = [s for s in STATIONS if s != VAL_STATION]
    print(f"Validation station: {VAL_STATION}")
    print(f"Training stations: {TRAIN_STATIONS}")

    # 准备文件列表
    train_files: List[Tuple[str, str]] = []
    val_files: List[Tuple[str, str]] = []
    all_days = [f"{i:04d}" for i in range(101, 132)]
    ...
    for day_folder in all_days:
        ...
        for file_name in sorted(os.listdir(h5_day_path)):
            if not file_name.endswith('.h5'):
                continue
            
            station_id = file_name.split('.')[0]
            h5_path = os.path.join(h5_day_path, file_name)
            tif_path = os.path.join(tif_day_path, file_name.replace('.h5', '.tif'))

            if not os.path.exists(tif_path):
                continue

            if station_id == VAL_STATION:
                val_files.append((h5_path, tif_path))
            elif station_id in TRAIN_STATIONS:
                train_files.append((h5_path, tif_path))

    print(f"Found {len(train_files)} training files and {len(val_files)} validation files.")

    # 全局归一化统计：在所有数据上计算，确保验证集与训练集归一化方式一致
    print("Computing global per-channel normalization stats (all stations)...")
    norm_stats = compute_global_channel_stats(all_days)
    
    print("Creating dataset...")
    # 使用文件列表直接创建数据集
    base_train = LidarCloudDataset(train_files, preload=True, augment=True, norm_stats=norm_stats)
    base_val = LidarCloudDataset(val_files, preload=True, augment=False, norm_stats=norm_stats)
